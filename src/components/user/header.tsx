    ...
    <>
      <header className="w-full py-4 md:py-6 px-4 md:px-8 max-w-7xl mx-auto flex items-center justify-between relative z-50">
        ...
        <div className="hidden md:flex items-center gap-3">
          <button
            onClick={openAuth}
            className="px-6 py-2.5 rounded-full bg-white/60 border border-white/80 shadow-sm text-slate-700 font-medium hover:bg-white hover:shadow-md transition-all backdrop-blur-sm cursor-pointer"
          >
            Log In
          </button>
        </div>
        ...
        {isMenuOpen && (
          <div className="absolute top-full left-0 w-full glass-panel border-t-0 rounded-t-none flex flex-col items-center py-6 gap-4 md:hidden shadow-xl">
            ...
            <button
              onClick={openAuth}
              className="btn-secondary w-2/3 text-center cursor-pointer"
            >
              Log In
            </button>
          </div>
