...
export default function ChatArea({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setIsLoading(true);
      // Завантажуємо повідомлення та дані партнера паралельно
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white/70 backdrop-blur-md rounded-[2rem] shadow-sm border border-white overflow-hidden">
      <div className="px-6 py-4 border-b border-pink-50/50 flex items-center justify-between bg-white/40">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        {isLoading ? (
          ...
          messages.map((msg) => (
            ...
            >
              <div
                className={`max-w-[80%] p-3 relative shadow-sm text-sm ${
                  msg.isMine
      ...
      <div className="p-4 bg-white/40 border-t border-pink-50/50">
        <div className="flex items-center gap-2 bg-white rounded-xl p-1.5 shadow-sm border border-slate-100 focus-within:ring-2 focus-within:ring-pink-500/10 transition-all">
          ...
          <button
            onClick={handleSend}
            className="p-2.5 bg-pink-500 hover:bg-pink-600 text-white rounded-lg transition-all active:scale-95 shadow-md shadow-pink-200"
          >
            <svg
